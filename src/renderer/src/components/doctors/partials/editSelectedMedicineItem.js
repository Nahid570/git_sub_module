...
const EditSelectedMedicineItem = ({
      ...
      return (
        <Fragment key={index}>
          <Button
            ...
            onClick={(e) => handleButtonInput(QTY_SCHEDULES[index], key)}
          >
  ...
  const handleButtonInput = (fieldPosition, index) => {
  ...
  const makeDefault = () => {
    const itemDetails = {
      ...
      quantitiesField: item?.quantitiesField,
  ...
  const setMakeDefaultData = (scheduleData) => {
    item.quantities = scheduleData?.quantities;
    item.quantitiesField = scheduleData?.quantitiesField;
    item.quantity = scheduleData?.quantity;
    item.quantityUnit = scheduleData?.quantityUnit;
    item.schedule = scheduleData?.schedule;
    item.scheduleUnit = scheduleData?.scheduleUnit;
    item.times = scheduleData?.times;
    item.timesUnit = scheduleData?.timesUnit;
    item.duration = scheduleData?.duration;
    item.durationUnit = scheduleData?.durationUnit;
    item.instructions = scheduleData?.instructions;
      ...
      return (
        ...
        >
          ...
          <Row className="mt-2 mb-1">
            ...
            >
              <Typeahead
                style={{ width: '89%', position: 'static' }}
                allowNew
                labelKey="name"
                ref={typeaheadRef}
  ...
  return (
    <>
      ...
      <MedicineDefaultModal
        medicineId={item?.id}
        setMakeDefaultData={setMakeDefaultData}
